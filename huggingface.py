...
class HFLM(TemplateLM):
    ...
    def _model_generate(
        ...
        **generation_kwargs: dict[str, Any],
        ...
        logits_processors: list[transformers.LogitsProcessor] = []

        # Top-H (alpha) — unchanged
        alpha = generation_kwargs.pop("alpha", None)
        if alpha is not None:
            logits_processors.append(
                TopH_LogitsProcessor(
                    top_n=100,
        ...
        # --- Top-W (w1) wiring: map CLI knobs and attach processor ---
        topw_keys = (
            "top_m",
            "lambda_geom",
            "warm_p",
            "use_geometry_for_sampling",
            "selection_temperature",
            "beta",
        )
        topw_args = {k: generation_kwargs.pop(k) for k in topw_keys if k in generation_kwargs}

        if "lambda_geom" in topw_args:
            # Use input embeddings (works with tied weights); w1 accepts torch.Tensor
            token_embs = self.model.get_input_embeddings().weight.detach().to(self.model.device)

            lam   = float(topw_args.get("lambda_geom", 5.0))
            warmp = float(topw_args.get("warm_p", 0.9))
            topm  = int(topw_args.get("top_m", 50))
            sel_T = float(topw_args.get("selection_temperature", generation_kwargs.get("temperature", 1.0)))
            beta  = float(topw_args.get("beta", 2.0))
            use_geom = bool(topw_args.get("use_geometry_for_sampling", False))
            proc = TopW_LogitsProcessor(
                token_embs,
                lam=lam,                     # w1 expects 'lam'
                init_top_p=warmp,            # w1 expects 'init_top_p'
                top_m=topm,
                beta=beta,
                # selection temperature inside Top-W; mirror HF sampling temp by default
                temperature=sel_T,
                use_geometry_for_sampling=use_geom,
            )
            # Minimal debug print of the *actual* values Top-W will use
            if not getattr(self, "_topw_printed", False):
                print(
                    f"[TopW:hf:init] lam={proc.lam}, beta={proc.beta}, "
                    f"warm_p={proc.init_top_p}, top_m={proc.top_m}, "
                     f"geom_chunk={proc.geom_chunk}, "
                    f"T_sel={proc.temperature}, alt_iters={proc.alt_iters}, "
                    f"geom_mode={proc.geom_mode}",
                    flush=True,
                )
                self._topw_printed = True
            logits_processors.append(proc)



            # 'use_geometry_for_sampling' is ignored by w1 (masking-only), safely popped above.
